...
export default function GuidesView() {
  ...
  const openAddModal = () => {
    ...
    if (guides.length >= limit) {
      if (confirm(`Límite de Plan ${currentPlan} (${limit} guías). ¿Ver planes?`)) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!fullName || !phone || !email) { alert('Completa todos los campos.'); return; }
  ...
  const handleToggleAdmin = async (g: Guide) => {
    if (!isAdmin) { alert('Solo admins pueden modificar privilegios.'); return; }
    if (!g.user_id) { alert('Solo guías vinculados pueden ser admins.'); return; }
    const targetMember = members.find(m => m.user.id === g.user_id);
    if (!targetMember) return;
    if (targetMember.user.id === currentUser?.id) { alert('No puedes cambiarte a ti mismo.'); return; }
    const nextRole = targetMember.member.role === AgencyRole.ADMIN ? AgencyRole.GUIA : AgencyRole.ADMIN;
    if (confirm(nextRole === AgencyRole.ADMIN ? `¿Ascender a ${g.full_name} a Admin?` : `¿Degradar a ${g.full_name} a Guía?`)) {
  ...
  const handleRemoveFromAgency = async (g: Guide) => {
    if (g.user_id && g.user_id === currentUser?.id) { alert('No puedes eliminarte a ti mismo.'); return; }
      ...
      <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-4 bg-white rounded-2xl p-5 border border-gray-100 shadow-card">
        <div>
          ...
          <div className="flex items-center gap-2 mt-2 text-[11px]">
            ...
            <button onClick={() => window.dispatchEvent(new Event('rumbo_open_pricing'))} className="text-pine font-bold hover:underline flex items-center gap-0.5">
        ...
        <div className="lg:col-span-4 flex flex-col gap-4">
          <div className="bg-white rounded-2xl border border-gray-150 p-5 shadow-sm">
            ...
            <div className="bg-sky/50 border border-sky/80 rounded-2xl p-4 flex items-center justify-between mt-3">
              ...
              <button onClick={handleCopyCode} className="px-4 py-2 bg-pine text-white font-semibold rounded-xl text-xs">
                  ...
                  <div key={g.id} className="p-4 bg-amber-50/20 border border-amber-200/50 rounded-2xl">
                    ...
                    {isAdmin && (
                      <div className="grid grid-cols-2 gap-1.5 mt-3">
                        <button onClick={() => handleRemoveFromAgency(g)} className="py-1.5 bg-gray-100 text-gray-800 rounded-lg text-[10px] font-bold"><UserX className="w-3 h-3 inline" /> Rechazar</button>
                        <button onClick={() => handleToggleActive(g)} className="py-1.5 bg-emerald-600 text-white rounded-lg text-[10px] font-bold"><Check className="w-3 h-3 inline" /> Aprobar</button>
                      </div>
                    )}
                  </div>
                ))}
                ...
                return (
                  <div key={g.id} className={`bg-white rounded-2xl border p-5 shadow-xs ${g.active ? 'border-gray-150' : 'border-gray-250 bg-gray-50/50 opacity-75'}`}>
                    <div className="flex justify-between items-start">
                      ...
                      {isAdmin && (
                        <div className="flex gap-0.5">
                          <button onClick={() => openEditModal(g)} className="p-1 text-gray-400 hover:text-pine"><Edit className="w-3.5 h-3.5" /></button>
                          <button onClick={() => handleRemoveFromAgency(g)} className="p-1 text-gray-400 hover:text-red-600"><Trash2 className="w-3.5 h-3.5" /></button>
                        </div>
                      )}
                    </div>
                    ...
                    {isAdmin && g.user_id && !isSelf && (
                      <button onClick={() => handleToggleAdmin(g)} className="mt-2 text-[10px] font-bold text-pine border border-gray-150 px-2 py-1 rounded-lg">
          ...
          <div className="relative w-full max-w-md bg-white rounded-2xl shadow-2xl p-6 z-10">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              ...
              <div className="flex justify-end gap-2">
                <button type="button" onClick={() => setIsModalOpen(false)} className="px-4 py-2 bg-gray-100 text-gray-700 text-xs font-semibold rounded-xl">Cancelar</button>
                <button type="submit" className="px-4 py-2 bg-pine text-white text-xs font-semibold rounded-xl">{editingGuide ? 'Guardar' : 'Crear'}</button>
              </div>
            </form>
          </div>
        </div>
