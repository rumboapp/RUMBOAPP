...
export default function DashboardView({ onNavigate }: DashboardViewProps) {
  ...
  const handleCreateDeparture = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newActivityId) { alert('Selecciona una actividad.'); return; }
  ...
  const handleAddPassenger = async (e: React.FormEvent) => {
    ...
    if (!paxName || !paxPhone) { alert('Nombre y celular son requeridos.'); return; }
  ...
  const handleDeletePassenger = async (paxId: string) => {
    if (confirm('¿Retirar a este pasajero?')) {
  ...
  const handleWeatherSuspension = async (depId: string) => {
    ...
    if (!act) return;
    if (confirm(`¿Suspender ${act.name} de las ${dep.departure_time}hs?`)) {
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-md p-4 flex flex-col gap-4">
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-gray-50 pb-3">
          ...
          <div className="flex gap-1.5">
            <button onClick={(e) => { e.stopPropagation(); exportToCSV(passengers, activeAct.name, dep.departure_date, activeGuideName); }}
              className="p-1.5 bg-gray-50 text-emerald-800 rounded-lg border text-[10px] font-semibold flex items-center gap-1">
              <FileSpreadsheet className="w-3.5 h-3.5" /> CSV
            </button>
            <button onClick={(e) => { e.stopPropagation(); printPDF(passengers, activeAct.name, dep.departure_date, activeGuideName, activeAct.meeting_point); }}
              className="p-1.5 bg-gray-50 text-ocean rounded-lg border text-[10px] font-semibold flex items-center gap-1">
                ...
                <div className="flex items-start justify-between gap-2">
                  <div className="flex gap-2">
                    <button onClick={(e) => { e.stopPropagation(); handleToggleCheckIn(p.id, p.checked_in); }}
                      className={`mt-0.5 w-4 h-4 rounded flex items-center justify-center border shrink-0 cursor-pointer ${p.checked_in ? 'bg-emerald-600 text-white' : 'border-gray-300 bg-white'}`}>
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    <button onClick={() => { navigator.clipboard.writeText(`${window.location.origin}/#/firma/${p.id}`); alert('Enlace copiado'); }}
                      className="p-1 rounded border bg-white hover:bg-gray-100 text-gray-600">
                    ...
                    {isAdmin && (
                      <button onClick={() => handleDeletePassenger(p.id)} className="p-1 text-gray-400 hover:text-red-650">
        ...
        {dep.status !== 'cancelada' && dep.status !== 'finalizada' && (
          <button onClick={(e) => { e.stopPropagation(); setIsAddPassengerOpen(true); }}
            className="mt-2 w-full py-2 border-2 border-dashed border-sky hover:border-pine text-ocean hover:text-pine font-semibold rounded-xl text-xs flex items-center justify-center gap-1">
    ...
    <div className="flex flex-col gap-6 w-full max-w-7xl mx-auto px-1 sm:px-4">
      <div className="flex flex-col md:flex-row gap-3 items-center justify-between bg-gradient-to-r from-pine to-emerald-900 rounded-2xl p-3.5 px-5 text-white shadow-md">
        <div className="flex items-center gap-3">
          <div className="p-2 bg-white/10 rounded-xl"><Compass className="w-5 h-5 text-emerald-400" /></div>
          <div>
            ...
            <h1 className="text-base font-display font-bold">Operaciones Diarias</h1>
          </div>
        </div>
      ...
      <div className="flex gap-2 overflow-x-auto pb-1">
        {[0, 1, 2, 3].map((offset) => {
          ...
          return (
            <button key={dateStr} onClick={() => { setSelectedDate(dateStr); setSelectedDeparture(null); }}
              className={`px-4 py-2.5 rounded-xl font-medium text-xs whitespace-nowrap cursor-pointer shrink-0 ${selectedDate === dateStr ? 'bg-pine text-white shadow-pop' : 'bg-white text-gray-600 border border-gray-150'}`}>
        ...
        <button onClick={() => setIsDatePickerOpen(true)}
          className="flex items-center gap-1.5 px-4 py-2.5 rounded-xl font-medium text-xs border cursor-pointer shrink-0 bg-white text-gray-600">
          ...
          <div className="flex justify-between items-center">
            ...
            {isAdmin && (
              <button onClick={() => { setNewDepartureDate(selectedDate); setIsAddDepartureOpen(true); }}
                className="flex items-center gap-1.5 px-3 py-1.5 bg-pine text-white rounded-xl text-xs font-semibold shadow-sm">
            ...
            filteredDepartures.map((dep) => {
              ...
              return (
                <div key={dep.id} onClick={() => { setSelectedDeparture(dep); db.getPassengersByDeparture(dep.id).then(setPassengers); }}
                  className={`bg-white rounded-2xl p-4 border transition-all cursor-pointer ${isSelected ? 'ring-2 ring-pine shadow-card' : 'border-gray-100 hover:border-pine/30'}`}>
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex gap-3">
                      <img src={act.photo_url} alt={act.name} className="w-16 h-16 rounded-xl object-cover shrink-0" />
                      <div>
                    ...
                    <div className="text-right">
                      ...
                      {dep.status !== 'cancelada' && weather?.condition === 'tormenta' && (
                        <button onClick={(e) => { e.stopPropagation(); handleWeatherSuspension(dep.id); }}
                          className="text-[9px] font-bold text-rose-700 bg-rose-50 px-1.5 py-0.5 rounded border border-rose-200 mt-2">
                  ...
                  <div className="flex items-center justify-between border-t border-gray-50 pt-3 mt-3">
                    <div className="flex items-center gap-1 text-xs text-gray-500">
                      ...
                      {isAdmin ? (
                        <button onClick={(e) => { e.stopPropagation(); setEditingGuidesDepId(editingGuidesDepId === dep.id ? null : dep.id); }}
                          className="font-medium text-gray-700 hover:text-pine border border-dashed border-gray-200 px-1.5 rounded">
                    ...
                    <div className="flex gap-1.5" onClick={(e) => e.stopPropagation()}>
                      {dep.status === 'programada' && (
                        <button onClick={() => handleUpdateStatus(dep.id, 'en_curso')} className="px-2 py-0.5 bg-blue-600 text-white rounded text-[10px] font-semibold">Comenzar</button>
                      )}
                      {dep.status === 'en_curso' && (
                        <button onClick={() => handleUpdateStatus(dep.id, 'finalizada')} className="px-2 py-0.5 bg-emerald-600 text-white rounded text-[10px] font-semibold">Finalizar</button>
                      )}
                    </div>
                  </div>
          ...
          <div className="relative w-full max-w-md bg-white rounded-2xl shadow-2xl p-6 z-10">
            ...
            <form onSubmit={handleCreateDeparture} className="flex flex-col gap-4">
              ...
              <div className="flex justify-end gap-2">
                <button type="button" onClick={() => setIsAddDepartureOpen(false)} className="px-4 py-2 bg-gray-100 text-gray-700 text-xs font-semibold rounded-xl">Cancelar</button>
                <button type="submit" className="px-4 py-2 bg-pine text-white text-xs font-semibold rounded-xl">Crear</button>
              </div>
            </form>
          </div>
        </div>
            ...
            <form onSubmit={handleAddPassenger} className="flex flex-col gap-4">
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="col-span-2">
                  <input type="tel" required placeholder="WhatsApp" value={paxPhone} onChange={(e) => setPaxPhone(e.target.value)}
                    className="w-full border border-gray-200 rounded-xl px-3 py-2 text-xs" />
                </div>
                <div>
              ...
              <div className="flex justify-end gap-2">
                <button type="button" onClick={() => setIsAddPassengerOpen(false)} className="px-4 py-2 bg-gray-100 text-gray-700 text-xs font-semibold rounded-xl">Cancelar</button>
                <button type="submit" className="px-4 py-2 bg-pine text-white text-xs font-semibold rounded-xl">Registrar</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {isBulkNotifyOpen && selectedDeparture && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          ...
          <div className="relative w-full max-w-xl bg-white rounded-3xl shadow-2xl p-6 z-10">
            ...
            <button onClick={() => setIsBulkNotifyOpen(false)} className="px-5 py-2.5 bg-gray-150 text-gray-800 font-bold text-xs rounded-xl">Cerrar</button>
          </div>
        </div>
      )}
      ...
      {isDatePickerOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          ...
          <div className="relative w-full max-w-sm bg-white rounded-3xl shadow-2xl p-6 z-10">
            ...
            <button onClick={() => setIsDatePickerOpen(false)} className="w-full py-2.5 bg-pine text-white font-bold text-xs rounded-xl">Aceptar</button>
          </div>
        </div>
      )}
    </div>
