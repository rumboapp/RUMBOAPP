...
function AppContent() {
  ...
  const [authView, setAuthView] = useState<'login' | 'register' | 'join-guide' | 'forgot' | 'reset' | 'awaiting-confirmation'>('login');
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email || !password) {
      notifyWarning('Por favor ingresa correo y contraseña.');
      return;
    }
  ...
  const handleAdminRegister = async (e: React.FormEvent) => {
    ...
    if (res.requiresConfirmation) {
      ...
      setAuthView('awaiting-confirmation');
    } else if (res.success) {
      navigateToHash('#/dashboard');
    } else {
      notifyError(res.error || 'Error al registrar.');
    }
  };
          ...
          <div className="w-full">
            ...
            {authView === 'awaiting-confirmation' && (
              <div className="flex flex-col gap-5 text-center items-center">
                ...
                <button onClick={() => { setAuthView('login'); navigateToHash('#/login'); }}
                  className="w-full py-3 bg-pine text-white rounded-xl text-xs font-bold shadow-md cursor-pointer hover:bg-pine-hover transition-colors">
