...
export default function ActivitiesView() {
  ...
  const openAddModal = () => {
    ...
    if (activities.length >= limit) {
      if (confirm(`Límite de Plan ${currentPlan} (${limit} actividades). ¿Ver planes?`)) {
    ...
    <div className="flex flex-col gap-6 w-full max-w-7xl mx-auto px-1 sm:px-4">
        ...
        <div>
          ...
          <div className="flex items-center gap-2 mt-2 text-[11px]">
            ...
            <button onClick={() => window.dispatchEvent(new Event('rumbo_open_pricing'))}
              className="text-pine font-bold hover:underline flex items-center gap-0.5">
              <Sparkles className="w-3.5 h-3.5 text-amber-500" /> Subir de Plan
            </button>
          </div>
        </div>
        {isAdmin && (
          <button onClick={openAddModal} className="flex items-center gap-1.5 px-4 py-2.5 bg-pine text-white rounded-xl text-xs font-semibold shadow-md">
          ...
          <div key={act.id} className={`bg-white rounded-2xl border overflow-hidden shadow-sm hover:shadow-card transition-all ${act.active ? 'border-gray-100' : 'border-gray-200 bg-gray-50/50 opacity-75'}`}>
            <div className="relative h-44 bg-gray-100 overflow-hidden">
              ...
              <button onClick={() => isAdmin && handleToggleActive(act)} disabled={!isAdmin}
                className={`absolute top-3 left-3 text-[10px] font-bold uppercase px-2.5 py-1 rounded-full shadow-sm ${act.active ? 'bg-emerald-600 text-white' : 'bg-gray-200 text-gray-600'}`}>
            ...
            <div className="p-4 flex-1 flex flex-col justify-between gap-4">
              ...
              {isAdmin && (
                <div className="flex gap-2 justify-end border-t border-gray-50 pt-3">
                  <button onClick={() => openEditModal(act)} className="p-2 bg-gray-50 text-ocean rounded-xl border"><Edit className="w-4 h-4" /></button>
                  <button onClick={() => handleDelete(act.id, act.name)} className="p-2 bg-gray-50 text-gray-400 hover:text-red-600 rounded-xl border"><Trash2 className="w-4 h-4" /></button>
                </div>
              )}
            </div>
          </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 overflow-y-auto">
          ...
          <div className="relative w-full max-w-lg bg-white rounded-3xl shadow-2xl p-6 z-10">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              ...
              <div className="flex justify-end gap-2">
                ...
                <button type="submit" className="px-4 py-2 bg-pine text-white text-xs font-semibold rounded-xl">{editingActivity ? 'Guardar' : 'Crear'}</button>
              </div>
            </form>
          </div>
        </div>
